/*
@brief This file refer a implementation of B-Tree.
@origin from site: touc.org/btree.html
@Chonghwashao
*/
...
template<class key, class payload> void Element<key, payload>::dump () {
    ...
    cout << "key=" << m_key << " m_payload="<< m_payload << " sub=" << mp_subtree << ' ';
...
void Node::insert_zeroth_subtree (Node* subtree) {
    ...
    if (subtree)
        ...
        subtree->mp_parent = this; // 记录下父节点指针@chonghwashao
